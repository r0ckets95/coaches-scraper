...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            tags = soup.select('[headers="col-fullname category-18"] a') or soup.select('.sidearm-roster-coach-link a')
            if tags:
                urls = [f"{self.base_url}{tag.get('href')}" for tag in tags]
            return urls
        except Exception as e:
            logging.error(f'Error extracting coach URLs - {e}')
            return None
    ...
    def get_name(self, soup):
        try:
            f_name = None
            l_name = None
            tag = soup.select_one('.sidearm-staff-member-bio-name') or soup.select_one('span.sidearm-coach-bio-name.sidearm-common-bio-name')
            if tag:
    ...
    def image_url(self, soup):
        try:
            tag = soup.select_one('.sidearm-staff-member-bio-image img')
            if tag:
