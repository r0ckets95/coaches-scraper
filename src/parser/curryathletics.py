...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            tags = soup.select('div.info p:first-child a')
            if tags:
    ...
    def get_title(self, soup):
        try:
            title = None
            tag = soup.find(lambda tag: tag.name == "td" and "Title" in tag.text)
            if tag:
    ...
    def get_email(self, soup):
        try:
            email = None
            tag = soup.find(lambda tag: tag.name == "td" and "Email" in tag.text)
            if tag:
                next_tag = tag.find_next('td')
    ...
    def get_number(self, soup):
        try:
            number = None
            tag = soup.find(lambda tag: tag.name == "td" and "Phone" in tag.text)
            if tag:
                next_tag = tag.find_next('td')
