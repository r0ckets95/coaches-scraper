...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            if tags:
                urls = [f"{tag.get('href')}" for tag in tags]
            return urls
        except Exception as e:
    ...
    def get_name(self, soup):
        try:
            f_name = None
            l_name = None
            tag = soup.select_one('.h2.site-title')
            if tag:
    ...
    def get_title(self, soup):
        try:
            title = None
            tag = soup.find(lambda tag: tag.name == "tr" and "Fonction" in tag.text)
            if tag:
    ...
    def image_url(self, soup):
        try:
            tag = soup.select_one('.banner__profile-picture')
            if tag:
