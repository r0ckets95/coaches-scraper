...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            tags = soup.select('div[class="info"] p a')
            if tags:
                urls = [f"{self.base_url}{tag.get('href')}" for tag in tags if 'mailto:' not in tag.get('href')]
            return urls
        except Exception as e:
    ...
    def get_title(self, soup):
        try:
            ...
            if tag:
                title = tag.select_one('.value').get_text().strip()
            return title
        except Exception as e:
    ...
    def get_email(self, soup):
        try:
            ...
            if tag:
                email = tag.select_one('.value').get_text().strip()
            return email
        except Exception as e:
    ...
    def get_number(self, soup):
        try:
            ...
            if tag:
                number = tag.select_one('.value').get_text().strip()
            return number
        except Exception as e:
