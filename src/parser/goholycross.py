...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            h3_tag = soup.find('caption', text="Women's Ice Hockey - Coaching Staff") or soup.find('caption', text="Men's Ice Hockey - Coaching Staff")
            if h3_tag:
                next_tag = h3_tag.find_next_sibling('tbody')
                tags = next_tag.select('a')
                if tags:
                    urls = [f"{self.base_url}{tag.get('href')}" for tag in tags]
            logging.info(f'Found coach URLs: {urls}')
            return urls
        except Exception as e:
