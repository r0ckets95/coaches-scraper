...
class Parser():
    ...
    def get_item(self, school, raw_html, url):
        ...
        try:
            ...
            logging.error(f"Error getting item from URL {url}: {e}")
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            h3_tag = soup.find('h2', text="Women's Ice Hockey Coaching Staff") or soup.find('h2', text="Men's Ice Hockey Coaching Staff")
            if h3_tag:
                next_tag = h3_tag.find_next_sibling()
                tags = next_tag.select('.staff-list-item__title-link') or next_tag.select('.roster-card-item__title-link')
                if tags:
                    urls = [f"{self.base_url}{tag.get('href')}" for tag in tags]
            logging.info(f"Found {len(urls)} coach URLs")
            return urls
        except Exception as e:
            logging.error(f"Error getting coaches URL: {e}")
            return None
    ...
    def image_url(self, soup):
        try:
            tag = soup.select_one('.roster-bio-photo.roster-bio-photo--staff img')
            if tag:
                return tag.get('src')
            return None
        except Exception as e:
