...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            h3_tag = soup.find('h3', text="Women's Hockey Coaching Staff") or soup.find('h3', text="Men's Hockey Coaching Staff")
            if h3_tag:
    ...
    def get_name(self, soup):
        try:
            f_name = None
            l_name = None
            tag = soup.select_one('.s-coaches-detail-page__coach__name span')
            if tag:
    ...
    def image_url(self, soup):
        try:
            tag = soup.select_one('[class="object-cover"]')
            if tag:
