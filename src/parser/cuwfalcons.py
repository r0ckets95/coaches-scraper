...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            tbody_tag = soup.select('[class="card-title"] a')
            if tbody_tag:
                for tag in tbody_tag:
                    if "https://www.cuwfalcons.com" not in tag['href']:
                        urls.append("https://www.cuwfalcons.com" + tag.get('href'))
                    else:
    ...
    def get_name(self, soup):
        try:
            f_name = None
            l_name = None
            tag = soup.select_one('.player-heading .name')
            if tag:
    ...
    def image_url(self, soup):
        try:
            ...
            if tag:
                image_url = f"https://www.cuwfalcons.com{tag.get('src')}"
                return image_url
            return None
        except Exception as e:
