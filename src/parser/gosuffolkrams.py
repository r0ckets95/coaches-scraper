...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            parent_tags = soup.select('tbody')
            if parent_tags:
                for p_tag in parent_tags:
                    tag = p_tag.select('a[href^="/sports/wice/coaches/"]') or p_tag.select('a[href^="/sports/mice/coaches/"]')
                    if tag:
                        urls.extend([f"{self.base_url}{tag.get('href')}" for tag in tag])
                        
            logging.info(f"Coaches URLs extracted: {urls}")
            return urls
        except Exception as e:
    ...
    def get_title(self, soup):
        try:
            ...
            if tag:
                title = tag.select_one('td').get_text().strip()
            return title
        except Exception as e:
    ...
    def get_email(self, soup):
        try:
            ...
            if tag:
                email = tag.select_one('td').get_text().strip()
            return email
        except Exception as e:
    ...
    def get_number(self, soup):
        try:
            ...
            if tag:
                number = tag.select_one('td').get_text().strip()
            return number
        except Exception as e:
