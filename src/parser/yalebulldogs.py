...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            h2_tag = soup.find(lambda tag: tag.name == 'h2' and "Women's Ice Hockey Coaching Staff" in tag.text) or \
                soup.find(lambda tag: tag.name == 'h2' and "Men's Ice Hockey Coaching Staff" in tag.text)
            if h2_tag:
                next_tag = h2_tag.find_next_sibling()
                tags = next_tag.select('tr th a')
                if tags:
