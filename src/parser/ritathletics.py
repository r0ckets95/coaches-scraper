...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            h3_tag = soup.find('h2', text="2024-25 Women's Hockey Coaching Staff") or soup.find('h2', text="2024-25 Men's Hockey Coaching Staff")
            if h3_tag:
                next_tag = h3_tag.find_next_sibling()
                tags = next_tag.select('.sidearm-coaches-coach th a')
                if tags:
                    urls = [f"{self.base_url}{tag.get('href')}" for tag in tags]
            return urls
        except Exception as e:
            logging.error(f'Error retrieving coaches URLs - {e}')
            return None
    ...
    def get_name(self, soup):
        try:
            f_name = None
            l_name = None
            tag = soup.select_one('.sidearm-coach-bio-name.sidearm-common-bio-name')
            if tag:
